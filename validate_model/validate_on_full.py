...
# ─── CONFIG
serve_URL      = "http://194.68.245.137:22119/v1"

# trained_model
CHAT_MODEL_PATH = (
    "/trained_model/snapshots/files"
)

# opensource embeddings model
EMBED_MODEL     = os.getenv("EMBED_MODEL", "models--dunzhang--stella_en_1.5B_v5/snapshots/b467445fc9c39af69fdb1bda9e18416df4d19f3c")
...
# ─── CLIENT SETUP
client     = OpenAI(api_key="EMPTY",       base_url=serve_URL)
    ...
    for gt,pr in zip(gt_reasons,pred_reasons):
        if not gt or not pr:
            ...
            try:
                ...
                print(f"Embedding error for '{gt}' vs '{pr}' in {path}: {e}")
                sims.append(0.0)
